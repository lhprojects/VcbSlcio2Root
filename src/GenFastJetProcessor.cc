...
GenFastJetProcessor aGenFastJetProcessor;

using namespace EVENT;

//ReconstructedParticleImpl* tempevent;

GenFastJetProcessor::GenFastJetProcessor() : Processor("GenFastJetProcessor")
{
...
GenFastJetProcessor::~GenFastJetProcessor()
{
}
...
void GenFastJetProcessor::initClusterMode()
{
...
void GenFastJetProcessor::initJetAlgo()
{
...
bool GenFastJetProcessor::isJetAlgo(string algo, int nrParams, int supportedModes)
{
...
void GenFastJetProcessor::initRecoScheme()
{
...
void GenFastJetProcessor::initStrategy()
{
 ...
 */
void GenFastJetProcessor::processEvent(LCEvent * evt)
{
    IMPL::LCCollectionVec* lccJetsOut = new IMPL::LCCollectionVec(LCIO::RECONSTRUCTEDPARTICLE);
    LCCollection* colctionIn = evt->getCollection(_lcParticleInName);
    int num_clction = colctionIn->getNumberOfElements();
    cout<<"num_clction : "<<num_clction<<endl;
    for(int num = 0; num < num_clction; num++){
        LCCollectionVec* tempClction = new LCCollectionVec( LCIO::RECONSTRUCTEDPARTICLE );
        ReconstructedParticle* col = dynamic_cast<ReconstructedParticle*>( colctionIn->getElementAt(num) );
        ReconstructedParticleVec components = col->getParticles();
        cout<<"components.size() : "<<components.size()<<endl;
        for(int i = 0; i<components.size(); i++){
            ReconstructedParticle* a_pfo = components.at(i);
            tempClction->addElement(a_pfo);
        }
        cout<<"tempClction->getNumberOfElements() : "<<tempClction->getNumberOfElements()<<endl;
        _pjList.clear();
        try{
            LCCollection* particleIn = tempClction;
        ...
        vector< fastjet::PseudoJet > jets;    jets.clear();    // will contain the found jets
...
//        tempevent = new ReconstructedParticleImpl();
//        tempevent->setEnergy( 999 );
//        double testmom[3] = {10, 10, 10};
//        tempevent->setMomentum( testmom );
...
//            ReconstructedParticle* rec = getRecPar( (*it), cs.constituents(*it) );
            ...
            fastjet::PseudoJet jet = (*it);
            a_Reco->setEnergy(jet.E());
            double mom[3] = {jet.px(), jet.py(), jet.pz()};
            a_Reco->setMomentum(mom);
            const vector< fastjet::PseudoJet >& constituents = cs.constituents(*it);
            for(unsigned int n = 0; n < constituents.size(); n++){
                ...
                a_Reco->addParticle( p );
            }
            lccJetsOut->addElement( a_Reco );

        }
//        cout<<"tempevent.size() : "<<tempevent->size()<<endl;
//        lccJetsOut->addElement( tempevent );
    }
        
	evt->addCollection(lccJetsOut, _lcJetOutName);
}

vector < fastjet::PseudoJet > GenFastJetProcessor::doIterativeInclusiveClustering()
{
...
EVENT::ReconstructedParticle* GenFastJetProcessor::getRecPar(fastjet::PseudoJet& fj, const vector< fastjet::PseudoJet >& constituents )
{
...
void GenFastJetProcessor::convertFromRecParticle(LCCollection* recCol)
{
