...
#include "RecoFastJetProcessor.h"
...
RecoFastJetProcessor aRecoFastJetProcessor;

using namespace EVENT;

RecoFastJetProcessor::RecoFastJetProcessor() : Processor("RecoFastJetProcessor")
{
	...
	registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "recParticleOut3", "a list of all reconstructed particles used to make jets. If no value specified collection is not created", _lcParticleOutName3, "");
	registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "recParticleOut4", "a list of all reconstructed particles used to make jets. If no value specified collection is not created", _lcParticleOutName4, "");
...
RecoFastJetProcessor::~RecoFastJetProcessor()
{
}
 ...
 */
void RecoFastJetProcessor::init()
{
...
//select the clustering mode, inclusive or exclusive with NJets or Pt_cut or En_cut
void RecoFastJetProcessor::initClusterMode()
{
	...
	// save the name of the cluster mode, inclusive or exclusive
...
void RecoFastJetProcessor::initJetAlgo()
{
	...
	if (isJetAlgo("ee_genkt_algorithm", 2, FJ_exclusive_nJets | FJ_exclusive_yCut))
	{
  ...
  /*  
    if(isJetAlgo("Valencia", 3, FJ_exclusive_nJets))
    {
        fastjet::contrib::ValenciaPlugin* p1;
        p1 = new fastjet::contrib::ValenciaPlugin(
        atof(_jetAlgoNameAndParams[1].c_str()),
        atof(_jetAlgoNameAndParams[2].c_str()),
//          );
        atof(_jetAlgoNameAndParams[3].c_str()));
        
        _jetAlgo = new fastjet::JetDefinition(p1);
    }
*/
...
bool RecoFastJetProcessor::isJetAlgo(string algo, int nrParams, int supportedModes)
{
...
void RecoFastJetProcessor::initRecoScheme()
{
...
void RecoFastJetProcessor::initStrategy()
{
 ...
 */
void RecoFastJetProcessor::processEvent(LCEvent * evt)
{
	...
	else if (_clusterMode == FJ_exclusive_nJets)
	{
		...
		else
		{
			...
			double y_cut = cs.exclusive_ymerge(_nrJets);
			cout<<"y_cut : "<<y_cut<<endl;
			double y_4 = cs.exclusive_ymerge(4);
			cout<<"y_4 : "<<y_4<<endl;
                        fv.push_back( cs.exclusive_ymerge(2) );
                        fv.push_back( cs.exclusive_ymerge(3) );
			fv.push_back( cs.exclusive_ymerge(4) );
			fv.push_back( cs.exclusive_ymerge(5) );
			fv.push_back( cs.exclusive_ymerge(6) );
		}
	}
	...
	*/
	...
	IMPL::LCCollectionVec* lccParticlesOut2(NULL);
        IMPL::LCCollectionVec* lccParticlesOut3(NULL);
        IMPL::LCCollectionVec* lccParticlesOut4(NULL);	
	if (_storeParticlesInJets){
	  ...
	  lccParticlesOut3= new IMPL::LCCollectionVec(LCIO::RECONSTRUCTEDPARTICLE);
          lccParticlesOut3->setSubset(true);
 	  lccParticlesOut4= new IMPL::LCCollectionVec(LCIO::RECONSTRUCTEDPARTICLE);
	  lccParticlesOut4->setSubset(true);
	...
	for (it=jets.begin(); it != jets.end(); it++)
	{
		...
		if (_storeParticlesInJets) 
		{
			for (unsigned int n = 0; n < cs.constituents(*it).size(); ++n)
			{
				...
				if( njet==2 )lccParticlesOut3->addElement( p );
				if( njet==3 )lccParticlesOut4->addElement( p );
			}
		}
		njet++;
	}
	...
	evt->parameters().setValues( "ycut", fv );
	...
	if (_storeParticlesInJets) evt->addCollection(lccParticlesOut3, _lcParticleOutName3);
	if (_storeParticlesInJets) evt->addCollection(lccParticlesOut4, _lcParticleOutName4);
...
//		lccJetParams.setValue(string("d_{n,n+1}"), (float)cs.exclusive_dmerge(nrJets));
                ...
                lccJetParams.setValue(string("y_two"),   (float)cs.exclusive_ymerge(2));
                lccJetParams.setValue(string("y_three"), (float)cs.exclusive_ymerge(3));          
                lccJetParams.setValue(string("y_four"),  (float)cs.exclusive_ymerge(4));
		lccJetParams.setValue(string("y_five"),  (float)cs.exclusive_ymerge(5));
                lccJetParams.setValue(string("y_six"),   (float)cs.exclusive_ymerge(6));
...
//		lccJetParams.setValue(string("y_{n,n+1}"), (float)cs.exclusive_ymerge(nrJets));
		...
		// Added from the original FastJetProcessor
//		lccJetParams.setValue(string("y_{n+1,n+2}"), (float)cs.exclusive_ymerge(nrJets+1));
//		lccJetParams.setValue(string("y_{n+2,n+3}"), (float)cs.exclusive_ymerge(nrJets+2));
//		lccJetParams.setValue(string("y_{n+3,n+4}"), (float)cs.exclusive_ymerge(nrJets+3));
//		lccJetParams.setValue(string("y_{n+4,n+5}"), (float)cs.exclusive_ymerge(nrJets+4));
	}


}

vector < fastjet::PseudoJet > RecoFastJetProcessor::doIterativeInclusiveClustering()
{
...
EVENT::ReconstructedParticle* RecoFastJetProcessor::getRecPar(fastjet::PseudoJet& fj, const vector< fastjet::PseudoJet >& constituents )
{
...
void RecoFastJetProcessor::convertFromRecParticle(LCCollection* recCol)
{
